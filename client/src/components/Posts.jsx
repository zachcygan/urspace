...
export default function Posts({ posts }) {
  ...
  useEffect(() => {
    if (data) {
      const me = data.me;
      console.log(me)
      setusersLikedPosts([...usersLikedPosts]);
  ...
  function LikeButton({ post }) {
    ...
    const likedButtonStyle = {
      backgroundcolor: isLiked ? 'red' : 'pink',
      color: 'white'
    };
    ...
    return (
      <button
        onClick={handleLikePost} style={likedButtonStyle}
        className="relative inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-br-lg border border-transparent py-4 text-sm font-semibold text-gray-900"> {isLiked ? 'Unlike' : 'Like'}
